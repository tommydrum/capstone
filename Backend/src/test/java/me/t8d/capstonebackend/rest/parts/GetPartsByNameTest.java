...
class GetPartsByNameTest {
    private GetPartsByName getPartsByName;
    private DB dbMock;

    @BeforeEach
    void setUp() throws SQLException {
        dbMock = DBMockProvider.getDBMock();
        getPartsByName = new GetPartsByName(dbMock);
    }
...
//        when(stringObjectMapMock.get("pathParameters")).thenReturn("{\"name\":\"test\"}");
        ApiGatewayResponse response = getPartsByName.handleRequest(stringObjectMapMock, contextMock);
